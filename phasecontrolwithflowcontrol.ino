...
int volumeTarget;
unsigned long extractionTimeTarget=30000;
...
//                  first_refresh_page((uint8_t)Nextion.read());  
                break;
                case 'N': 
                    receive_number((uint8_t)Nextion.read(),(uint8_t)Nextion.read()); // printh 23 03 4e xx xx  
                break;
                case 'S': 
                    receive_state((uint8_t)Nextion.read(),(uint8_t)Nextion.read());  // printh 23 03 53 xx xx
                break;
                case 'E': //event
...
void receive_state(byte nextion_attr, byte state)
{
...
void receivePreinfusionSetting(bool val)
{
  val = preinfusionSetting;
}

void receivePreinfusionTime(int val)
{
  val = preinfusionTime;
}

void receiveVolumeTarget(int val)
{
  val = volumeTarget;
}
...
void brewControl()
{
  if(brewing == true)
  {
    ...
    {
      if(extracting==false)preinfusionStop();
      if(millis() - extractionStartTime < extractionTimeTarget )
      {
        extraction();
      }
      else
...
void extraction()
{
  ...
  if(millis() - brewStartTime < extractionTimeTarget)flowSetpoint = (/*volumeTarget*/60 - millilitersBrewed) / (float)((extractionTimeTarget - (millis() - brewStartTime))/1000);
}
